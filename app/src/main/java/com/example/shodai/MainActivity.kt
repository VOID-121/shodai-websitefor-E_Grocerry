    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        nav_view.setNavigationItemSelectedListener {
            when(it.itemId){
                ...
                R.id.action_home -> startActivity(Intent(this@MainActivity, MainActivity::class.java))
                R.id.cart -> startActivity(Intent(this@MainActivity, Cart::class.java))
                R.id.category -> startActivity(Intent(this@MainActivity, categories::class.java))
            }
            it.isChecked = true
            true
        }
            ...
            override fun onDataChange(p0: DataSnapshot) {
                for(i in p0.children){
                   val pro = i.getValue(Product::class.java)
                    if (category != null){
                        if (pro!!.category == category.toString()){
                            products.add(pro!!)
                        }
                    }else{
                        products.add(pro!!)
